...
import type { Area, Chapter } from "@/lib/journey/areas";
import type { Lang } from "@/lib/journey/i18n";
import type { Theme } from "@/lib/journey/theme";
import { StarField } from "./StarField";
import { ThemeToggle } from "./ThemeToggle";
import { LangToggle } from "./LangToggle";

type Props = { areas: Area[]; lang: Lang; focusSlug?: string; theme: Theme; toggleTheme: () => void };
...
export function CosmicView({ areas, lang, focusSlug, theme, toggleTheme }: Props) {
    ...
    <div data-theme={theme} style={{ background: "var(--bg)", height: "100dvh", overflow: "hidden", position: "relative" }}>
          ...
          return (
            <div key={area.id} style={{
              ...
              <GalaxySvg
                ...
                onClickArea={() => isFocused ? router.push(`/journey/${area.slug}`) : rotateTo(i)}
                onClickChapter={(ch) => isFocused ? router.push(`/journey/${area.slug}/${ch.slug}`) : rotateTo(i)}
              />
            </div>
          );
        })}
      ...
      <div style={{
        ...
        <InlineSearch
          ...
          onPick={(area, chapter) => router.push(`/journey/${area.slug}/${chapter.slug}`)} />
      </div>
