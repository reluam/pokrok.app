...
export async function syncAuthedUser(): Promise<{ clerkId: string; userId: string } | null> {
  // Účty schované (lib/features.ts) nebo Clerk bez klíčů → všichni jsou anonymní;
  // auth() by bez clerkMiddleware stejně spadlo.
  if (!ACCOUNTS_ENABLED || !process.env.CLERK_SECRET_KEY) return null;
