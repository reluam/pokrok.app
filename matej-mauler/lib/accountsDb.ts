...
registerSchema({
  ...
  statements: (sql) => [
    ...
    sql`CREATE TABLE IF NOT EXISTS anonymous_sessions (
      id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      created_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
    ...
    sql`CREATE INDEX IF NOT EXISTS participations_user_idx ON participations(user_id)`,
    sql`CREATE INDEX IF NOT EXISTS participations_session_idx ON participations(session_id)`,
    sql`CREATE INDEX IF NOT EXISTS participations_exp_idx ON participations(experiment_slug)`,
    ...
    sql`CREATE TABLE IF NOT EXISTS xp_events (
      id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      user_id         UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      experiment_slug TEXT,
    ...
    sql`CREATE INDEX IF NOT EXISTS xp_events_user_idx ON xp_events(user_id)`,

    // Seed badge catalog from code (single source of truth).
    ...ALL_BADGES.map((b) => sql`INSERT INTO badges (slug, experiment_slug, name, description, criteria_key, xp_value)
