...
async function getWeekSessions(userId: string, projectIds: string[]): Promise<Session[]> {
  try {
...
async function getWeekBookings(userId: string, projectIds: string[]): Promise<Booking[]> {
  if (projectIds.length > 0) {
    const rows = await sql`
      SELECT b.id, b.scheduled_at, b.name, b.email, e.project_id
      FROM bookings b
      LEFT JOIN events e ON e.id = b.event_id
      WHERE b.user_id = ${userId}
        AND b.status != 'cancelled'
        AND b.scheduled_at::date >= date_trunc('week', CURRENT_DATE)
        AND b.scheduled_at::date < date_trunc('week', CURRENT_DATE) + INTERVAL '7 days'
...
async function getWeekLeads(userId: string, projectIds: string[]): Promise<Lead[]> {
  if (projectIds.length > 0) {
    const rows = await sql`
...
export default async function OverviewWeekPage({ searchParams }: PageProps) {
  ...
  const [sessions, bookings, leads] = await Promise.all([
    getWeekSessions(userId, projectIds),
    getWeekBookings(userId, projectIds),
    getWeekLeads(userId, projectIds)
  ]);
