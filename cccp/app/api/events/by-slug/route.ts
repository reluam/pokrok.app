...
export async function GET(request: Request) {
  ...
  const eventRows = await sql`
    SELECT e.id, e.user_id, e.slug, e.name, e.duration_minutes,
           COALESCE(e.one_booking_per_email, false) AS one_booking_per_email,
    ...
    LIMIT 1
  ` as { id: string; user_id: string; slug: string; name: string; duration_minutes: number; one_booking_per_email: boolean; first_day_of_week: number }[];
  ...
  return NextResponse.json({
    ...
    one_booking_per_email: event.one_booking_per_email ?? false,
