...
function DashboardContent() {
  ...
  useEffect(() => {
    ...
    const sync = async (type: string, data: unknown) => {
      if (!data) return;
      try { await fetch("/api/manual/user-context", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type, data }) }); } catch {}
    };
  ...
  return (
    <main className="min-h-screen py-16 md:py-24 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-foreground">Manuál</h1>
