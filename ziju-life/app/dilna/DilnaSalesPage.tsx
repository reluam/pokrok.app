...
const faqs = [
  ...
  {
    q: "Pro koho to je?",
    a: "Pro tebe, pokud chceš žít vědoměji. Ať už hledáš směr, chceš si pojmenovat hodnoty, nebo potřebuješ parťáka na přemýšlení — Dílna je místo, kde experimentuješ sám se sebou.",
  },
  {
    q: "Co dostanu?",
...
function MagicLinkModal({ onClose }: { onClose: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/auth/magic-link", {
        ...
        body: JSON.stringify({ email, next: "/dilna/dashboard" }),
      });
  ...
  async function verifyCode(code: string) {
    ...
    try {
      ...
      window.location.href = "/dilna/dashboard";
    } catch {
...
function DilnaContent() {
  ...
  async function handleBuy() {
    ...
    setBuyError("");
    try {
      const res = await fetch("/api/dilna/checkout", { method: "POST" });
      const data = await res.json();
      if (data.url) {
...
export default function DilnaSalesPage() {
  return (
    <Suspense
      fallback={
