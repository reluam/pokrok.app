...
export const metadata: Metadata = {
  title: "Kompas | Dílna | Žiju life",
  description:
    "Projdi životní cestu zastávku po zastávce. Sedm kroků, cvičení, šablony a materiály — vše v jednom průvodci.",
};

export default async function DilnaTvujKompasPage({
  searchParams,
}: {
  searchParams: Promise<{ payment?: string }>;
}) {
  // Dílna access check — no subscription → redirect to sales page
  const hasAccess = await checkDilnaAccess();
  if (!hasAccess) redirect("/dilna");
