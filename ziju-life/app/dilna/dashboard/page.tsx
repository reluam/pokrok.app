...
const CoachingChatPanel = dynamic(() => import("@/components/dilna/CoachingChatPanel"), { ssr: false });
const PrioritiesWidget = dynamic(() => import("@/components/dilna/PrioritiesWidget"), { ssr: false });
const DailyTodosWidget = dynamic(() => import("@/components/dilna/DailyTodosWidget"), { ssr: false });
const RitualsChecklistWidget = dynamic(() => import("@/components/dilna/RitualsChecklistWidget"), { ssr: false });
...
function NastavSiDenTab({ selection, onSave, onComplete, onReset }: {
  ...
  const loadTracker = useCallback(async () => {
    try {
      const res = await fetch("/api/dilna/ritual-completions");
      if (res.ok) {
  ...
  const toggleDay = async (ritualId: string, date: string) => {
    ...
    try {
      await fetch("/api/dilna/ritual-completions", {
        method: "POST",
...
function DashboardContent() {
  ...
  // Auth check
  useEffect(() => {
    fetch("/api/dilna/check")
      .then((r) => r.json())
      .then((d) => {
        if (!d.valid) { router.replace("/dilna"); } else {
          setEmail(d.email ?? "");
          setChecked(true);
        }
      })
      .catch(() => router.replace("/dilna"));
  }, [router]);
  ...
  useEffect(() => {
    ...
    const sync = async (type: string, data: unknown) => {
      if (!data) return;
      try { await fetch("/api/dilna/user-context", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type, data }) }); } catch {}
    };
  ...
  const goToTab = useCallback((tab: string) => {
    ...
    router.push(`/dilna/dashboard${query}`, { scroll: false });
  }, [router]);
  ...
  const handleCheckinSave = useCallback(async (
    ...
    try {
      const res = await fetch("/api/dilna/checkin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <main className="min-h-screen py-16 md:py-24 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-foreground">Dílna</h1>
