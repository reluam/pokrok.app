...
export default function LaboratorGrantsContent() {
  ...
  const loadGrants = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/laborator-grants");
  ...
  async function handleDelete(id: string, email: string) {
    ...
    setDeletingId(id);
    try {
      await fetch(`/api/admin/laborator-grants?id=${id}`, { method: "DELETE" });
    ...
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
          ...
          Laboratoř — přístupy zdarma
        </h1>
        <p className="text-sm text-foreground/60 mt-1">
          Udělej konkrétnímu e-mailu přístup do Laboratoře bez placení.
        </p>
      </div>
