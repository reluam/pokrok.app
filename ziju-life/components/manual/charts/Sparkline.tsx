...
export function AreaSparklines({ checkins }: { checkins: CheckinEntry[] }) {
  ...
  return (
    <div>
      <p className="text-lg text-foreground/40 mb-3">Vývoj oblastí (posledních {withAreas.length} týdnů)</p>
        ...
        {WHEEL_AREAS.map((area) => {
          ...
          return (
            <div key={area.key} className="flex items-center gap-2">
              <div className="w-14 text-base text-foreground/50 font-medium truncate">{area.short}</div>
              ...
              <div className="flex items-baseline gap-0.5">
                <span className="text-lg font-bold text-foreground/70">{last}</span>
                {delta !== 0 && (
                  <span className={`text-base font-semibold ${delta > 0 ? "text-green-500" : "text-red-400"}`}>
                    {delta > 0 ? `+${delta}` : delta}
                  </span>
                )}
              </div>
