...
export default function CoachingChatPanel() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch("/api/manual/coaching-chat");
        if (res.ok) {
          const data = await res.json();
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch("/api/manual/coaching-chat", {
        ...
        body: JSON.stringify({ message: text, pageContext: pathname || "/" }),
      });
      const data = await res.json();
  ...
  return (
    <div className="bg-white border border-black/8 rounded-[24px] flex flex-col h-[calc(100dvh-220px)] min-h-[400px] max-h-[800px]">
      {/* Header */}
      ...
      {!loading && (
        <>
          ...
          <button
            type="button"
            onClick={() => booking?.openBookingPopup()}
