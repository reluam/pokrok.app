    ...
    return (
      <div className="max-w-2xl mx-auto space-y-5">
          ...
          {filled.map((a, i) => {
            ...
            return (
              <div key={origIdx} className="bg-white border border-black/8 rounded-[20px] px-4 py-4 space-y-2">
                <p className="font-medium text-base text-foreground">{a.name}</p>

                <div className="flex items-center gap-2">
                  <span className="text-sm text-red-400 w-8">-5</span>
                  ...
                  <span className="text-sm text-green-500 w-8 text-right">+5</span>
                </div>
                ...
                <select
                  ...
                  className="text-sm px-3 py-1.5 border border-black/10 rounded-lg bg-white text-foreground/60"
                >
        ...
        <div className="flex gap-2">
          <button onClick={() => setStep("list")} className="flex-1 py-2.5 border border-foreground/15 text-foreground/60 rounded-full font-semibold text-base">
            ← Zpět
          </button>
          <button onClick={() => setStep("reflect")} className="flex-1 py-2.5 bg-accent text-white rounded-full font-bold text-base hover:bg-accent-hover transition-colors">
            Dál — reflexe →
          </button>
        </div>
      </div>
    ...
    return (
      <div className="max-w-2xl mx-auto space-y-5">
        <div>
          ...
          <p className="text-base text-foreground/55 mt-1">Co ti dává energii a co ji bere?</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="px-4 py-4 rounded-2xl bg-green-50 border border-green-200 space-y-2">
            <p className="text-sm font-bold text-green-700 uppercase tracking-wider">Energizéry ⚡</p>
            {energizers.length > 0 ? energizers.map((a, i) => (
              <p key={i} className="text-base text-green-800">+{a.rating} {a.name}</p>
            )) : <p className="text-sm text-green-600/60">Žádné pozitivní aktivity</p>}
          </div>
          <div className="px-4 py-4 rounded-2xl bg-red-50 border border-red-200 space-y-2">
            <p className="text-sm font-bold text-red-700 uppercase tracking-wider">Vampýři 🧛</p>
            {drainers.length > 0 ? drainers.map((a, i) => (
              <p key={i} className="text-base text-red-800">{a.rating} {a.name}</p>
            )) : <p className="text-sm text-red-600/60">Žádné vyčerpávající aktivity</p>}
          </div>
        </div>

        <div className="space-y-3">
          <div className="space-y-1.5">
            <label className="text-base font-medium text-foreground/70">Co si uvědomuješ? Jaké vzorce vidíš?</label>
            <textarea
              value={data.insights}
          ...
          <div className="space-y-1.5">
            <label className="text-base font-medium text-foreground/70">Jak by vypadal tvůj ideální energetický týden?</label>
            <textarea
        ...
        <div className="flex gap-2">
          <button onClick={() => setStep("rate")} className="flex-1 py-2.5 border border-foreground/15 text-foreground/60 rounded-full font-semibold text-base">
            ← Zpět
          </button>
          <button
            onClick={async () => {
