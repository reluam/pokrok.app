...
function ViewMode({ data }: { data: RelationshipMapData }) {
  ...
  return (
    <div className="space-y-2">
      <p className="text-sm text-foreground/40">{people.length} lidí zmapováno</p>
      {(["inner", "middle", "outer"] as const).map((c) => {
        if (byCircle[c].length === 0) return null;
        return (
          <div key={c}>
            <p className="text-xs uppercase tracking-wider text-foreground/30 font-semibold">{CIRCLE_LABELS[c]}</p>
            <div className="flex flex-wrap gap-1 mt-0.5">
              {byCircle[c].map((p, i) => (
                <span key={i} className={`text-sm px-2 py-0.5 rounded-full ${p.energizes ? "bg-green-50 text-green-700" : "bg-red-50 text-red-600"}`}>
                  {p.name}
                </span>
              ))}
            </div>
        ...
        {people.map((p, i) => (
          <div key={i} className="flex gap-1.5 items-center">
            ...
            <select
              ...
              className="w-20 text-sm rounded-lg border border-black/[0.08] bg-white/70 px-1 py-1.5 text-foreground/70 focus:outline-none"
            >
            ...
            <button
              ...
              className={`text-base w-7 h-7 rounded-full flex-shrink-0 ${p.energizes ? "bg-green-50 text-green-600" : "bg-red-50 text-red-500"}`}
            >
              {p.energizes ? "+" : "-"}
            </button>
          </div>
        ))}
        <button onClick={addPerson} className="text-sm text-accent hover:opacity-80">+ Přidat</button>
      </div>
      <textarea
        value={insights}
