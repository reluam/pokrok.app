...
export function EmptyCta({ emoji, title, description, buttonLabel, onClick, href }: {
  ...
  const cls = "inline-flex items-center gap-2 px-5 py-2.5 bg-accent text-white rounded-full font-semibold text-base hover:bg-accent-hover transition-colors";
  return (
    <div className="bg-white border border-black/8 rounded-[24px] px-6 py-8 text-center space-y-3">
      <p className="text-4xl">{emoji}</p>
      <div>
        <p className="font-bold text-foreground">{title}</p>
        <p className="text-base text-foreground/55 mt-1">{description}</p>
      </div>
