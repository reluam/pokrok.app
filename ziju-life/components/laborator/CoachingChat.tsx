...
import { Send, CalendarPlus, Sparkles } from "lucide-react";
    ...
    <div className="flex flex-col h-[calc(100vh-80px)]">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-black/5">
        <div className="flex items-center gap-2.5">
          ...
          <h1 className="text-lg font-bold text-foreground">AI Kouč</h1>
        </div>
        {budgetPct !== null && (
      ...
      {/* Input */}
      {error !== "no_budget" && (
        <div className="flex items-end gap-2 px-4 py-3 border-t border-black/5 bg-white/80">
          <textarea
            ref={inputRef}
            value={input}
      ...
      <Link
        ...
        className="flex items-center justify-center gap-2 px-4 py-3 border-t border-black/5 bg-white/50 hover:bg-accent/5 transition-colors"
      >
        <CalendarPlus size={16} className="text-accent" />
        <span className="text-sm font-semibold text-accent">Objednat koučovací sezení</span>
      </Link>
    </div>
  );
}
