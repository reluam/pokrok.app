...
export async function getLeadById(id: string): Promise<Lead | null> {
  const rows = await sql`
    ...
    WHERE id = ${id}
    LIMIT 1
  ` as { id: string; email: string; name: string | null; source: string; status: string; message: string | null; utm_source: string | null; utm_medium: string | null; utm_campaign: string | null; created_at: Date; updated_at: Date }[]
  const r = rows[0]
  if (!r) return null
  return {
    id: r.id,
