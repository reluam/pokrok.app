...
/**
 * Server-side check: verifies active Manuál access.
 *
 ...
 */
export async function checkManualAccess(emailOverride?: string): Promise<boolean> {
  ...
  if (stripeSecretKey) {
    try {
      ...
      console.error("[manual-auth] Stripe fallback error:", err);
    }
  }
