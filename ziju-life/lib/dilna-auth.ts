...
/**
 * Server-side check: verifies active Dílna access.
 *
 ...
 */
export async function checkDilnaAccess(emailOverride?: string): Promise<boolean> {
  ...
  if (stripeSecretKey) {
    try {
      ...
      console.error("[dilna-auth] Stripe fallback error:", err);
    }
  }
