...
export async function sendAuditZivotaAccessEmail(
  ...
  const siteUrl = getSiteUrl()
  const next = encodeURIComponent('/tvuj-kompas')
  ...
  const content = `
    ...
    <p style="color: ${TEXT_DARK}; font-size: 16px; line-height: 1.6; margin: 0 0 8px;">
      Platba proběhla úspěšně — Tvůj kompas je tvůj!
    </p>
  ...
  try {
    const { error } = await resend.emails.send({
      from,
      to: [to],
      subject: 'Tvůj kompas — tvůj přístup je ready',
...
export async function sendContentRequestEmail(
  ...
  const content = `
    <p style="color: ${TEXT_DARK}; font-size: 16px; line-height: 1.6; margin: 0 0 16px;">
      AI průvodce v Manuálu nedokázal pomoct uživateli s tímto tématem:
    </p>
