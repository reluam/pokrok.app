    ...
    <div className="bg-white bg-opacity-95 rounded-2xl p-8 border border-orange-200 shadow-xl backdrop-blur-sm w-full max-w-4xl mx-auto">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-2xl font-bold text-orange-800">🌅 Pohled za dneškem</h2>
            <p className="text-gray-600 mt-1">Reflexe nad uplynulým dnem</p>
          </div>
          <button
            onClick={handleSkip}
      ...
      <div className="space-y-6">
        ...
        <div>
          <label className="block text-lg font-bold text-gray-900 mb-2">
            1. Co se dnes povedlo? ✨
          </label>
          <textarea
            value={whatWentWell}
            onChange={(e) => setWhatWentWell(e.target.value)}
            placeholder="Zapište si, co se vám dnes povedlo nebo co vás potěšilo..."
        ...
        <div>
          <label className="block text-lg font-bold text-gray-900 mb-2">
            2. Co se dnes nepovedlo? 🤔
          </label>
          <textarea
            value={whatWentWrong}
            onChange={(e) => setWhatWentWrong(e.target.value)}
            placeholder="Zapište si, co se nepovedlo nebo co byste chtěli příště udělat jinak..."
            className="w-full h-32 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent resize-none"
          />
        </div>
              ...
              <div className="space-y-3">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm text-gray-700">Pokrok</span>
            ...
            <div>
              {activeGoals.length === 0 ? (
                <div className="bg-gray-50 rounded-lg p-4 text-center text-gray-500">
                  Nemáte žádné aktivní cíle k aktualizaci
                </div>
              ) : (
                <>
                  {showGoalSelector ? (
                        ...
                        {activeGoals.map(goal => (
                          ...
                          >
                            ...
                            <div className="text-sm text-gray-500 mt-1">
                              Současný pokrok: {goal.progress || 0}%
                            </div>
                          </button>
                        ))}
                      </div>
