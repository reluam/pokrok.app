    ...
    for (const area of areas) {
      try {
        ...
        if (nameNeedsEncryption) {
          encryptedName = encryptText(area.name, area.user_id)
        }
        
        if (descriptionNeedsEncryption) {
          encryptedDescription = encryptText(area.description, area.user_id)
        }
        ...
        errorCount++
        errors.push(`Area ${area.id}: ${error.message || error}`)
      }
    }
