...
function SortableStep({ step, index, onRemove, isCurrent, t }: { 
  step: DailyStep, 
  index: number, 
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {isCurrent && <span className="text-xs bg-purple-500 text-white px-2 py-1 rounded">{t('dailyPlanning.current')}</span>}
          <button
            onClick={() => onRemove(step.id)}
            className="px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600 transition-colors"
          >
            {t('dailyPlanning.remove')}
          </button>
        </div>
      </div>
      <div className="mt-2 text-xs text-gray-500">
        ⏱️ {step.estimatedTime}{t('dailyPlanning.estimatedTime')}
      </div>
    </div>
  )
}
  ...
  const handleCreateNewStep = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Goals removed - no selectedGoalId to reset
      }
    } catch (error) {
